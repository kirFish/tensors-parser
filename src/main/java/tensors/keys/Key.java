...
public class Key {
    ...
    /**
     * indexes say the exact place in tensor:
     ...
     *
     * */
    public static Key[] initializeKeys(int[] indexes, Key[] keys) {
         ...
         */
        FirstKey firstKey = new FirstKey(indexesString[1]);
        SecondKey secondKey;
        ThirdKey thirdKey;
        ...
        switch (keys.length){
            case 2:
                secondKey = new SecondKey(indexesString[2]);
                return new Key[]{firstKey,secondKey};
            case 3:
                secondKey = new  SecondKey(indexesString[2]);
